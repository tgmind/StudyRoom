...
describe("GoalsGuideModal Component", () => {
  it("renders educational content about rolling 20h goals and append-only rules", () => {
    render(<GoalsGuideModal isOpen={true} onClose={vi.fn()} />);

    expect(screen.getByText(/Rolling 20-Hour Goals Guide/i)).toBeInTheDocument();
    expect(screen.getByText(/Continuous 20-Hour Windows/i)).toBeInTheDocument();
